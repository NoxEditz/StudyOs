...
function logMood(btn,mood){const today=todayStr();const logs=DB.get('mood_logs')||{};logs[today]=mood;DB.set('mood_logs',logs);document.querySelectorAll('.mood-btn').forEach(b=>b.classList.toggle('selected',b.dataset.mood===mood));showToast(`${mood} تم تسجيل مزاجك`);addXP(5,'تسجيل المزاج');tryUnlock('mood_tracker');renderMood();queueCloudBackup();}
function renderMood(){
...
function importData(){const inp=document.createElement('input');inp.type='file';inp.accept='.json';inp.onchange=e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=r=>{try{const data=JSON.parse(r.result);if(typeof data!=='object'||!data)throw new Error();Object.entries(data).forEach(([k,v])=>DB.set(k,v));showToast('📥 تم الاستيراد','green');renderActivePage();queueCloudBackup();}catch{showToast('⚠️ ملف غير صالح','red');}};reader.readAsText(file);};inp.click();}
function clearData(){if(!confirm('هل أنت متأكد من حذف كل البيانات؟'))return;['sessions','subjects','flashcards','notes','habits','habit_logs','gam','achievements','settings','mood_logs','kanban_tasks','tip_today','tip_date'].forEach(k=>DB.del(k));showToast('🗑️ تم الحذف');queueCloudBackup().then(()=>location.reload());}
...
let _cloudBackupTimer=null,_cloudRestoreDone=false,_cloudBusy=false, _supabaseInstance=null;

function cloudClient(){
  if(!window.supabase?.createClient)return null;
  if(!_supabaseInstance){
    _supabaseInstance = window.supabase.createClient(CLOUD_CFG.url,CLOUD_CFG.anonKey);
  }
