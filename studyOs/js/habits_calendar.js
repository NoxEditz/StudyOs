...
function toggleHL(id,date){const logs=DB.get('habit_logs')||{};const key=`${id}_${date}`;logs[key]=!logs[key];DB.set('habit_logs',logs);if(logs[key])addXP(5,'عادة مكتملة');renderHabits();}
const delHabit=id=>{const h=(DB.get('habits')||[]).filter(x=>x.id!==id);DB.set('habits',h);showToast('🗑️ تم الحذف');renderHabits();};
