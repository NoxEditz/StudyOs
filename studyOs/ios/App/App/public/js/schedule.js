...
function toggleSession(idx){const sessions=DB.get('sessions')||[];if(!sessions[idx])return;sessions[idx].done=!sessions[idx].done;DB.set('sessions',sessions);if(sessions[idx].done)addXP(15,'جلسة مكتملة');renderActivePage();}
function deleteSession(idx){const sessions=DB.get('sessions')||[];sessions.splice(idx,1);DB.set('sessions',sessions);showToast('🗑️ تم الحذف');renderActivePage();}
...
function toggleChap(subj,idx){
  ...
  sd[subj][idx].done=!sd[subj][idx].done;DB.set('subjects',sd);addXP(5,'فصل مكتمل');
  if(sd[subj].every(c=>c.done)){tryUnlock('complete_subject');fireConfetti();}renderActivePage();
}
